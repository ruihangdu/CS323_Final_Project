...
import { Activity, TrendingUp, ChevronRight, Building2, Palette } from "lucide-react";
...
export default function SetupPage() {
  ...
  const effectiveCompany = company.trim() || scenario?.defaultCompany || "";
  const effectiveColor = selectedColor ?? (scenario ? { hsl: scenario.defaultColor, fg: scenario.defaultFg, label: "" } : COLOR_PRESETS[0]);

  const canLaunch = !!selectedScenario;
  ...
  function handleLaunch() {
    ...
    if (scenario.path.startsWith("/cos-simulator")) {
      window.location.href = `${scenario.path}?${params.toString()}`;
    } else {
      navigate(`${scenario.path}?${params.toString()}`);
    }
  }
    ...
    <div className="min-h-screen w-full bg-background text-foreground font-sans flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        <div className="w-full max-w-3xl flex flex-col gap-10">
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex flex-col gap-1">
                        <span className={`font-bold text-sm ${active ? "text-primary" : "text-foreground"}`}>
                          {s.title}
                        </span>
                          ...
                          {s.tags.map((tag) => (
                            <span
                              ...
                              className={`text-[10px] font-mono px-2 py-0.5 rounded border ${
                                active ? "border-primary/40 text-primary bg-primary/10" : "border-border text-muted-foreground"
                              }`}
                            >
                              {tag}
                            </span>
          ...
          {/* Step 2 + 3: Branding (only shown after scenario selected) */}
          {selectedScenario && (
            <>
              <section>
                <StepLabel step={2} label="Set your company name" />
                <div className="mt-4 flex items-center gap-3">
                  <Building2 className="w-4 h-4 text-muted-foreground shrink-0" />
                  <Input
                    placeholder={scenario?.defaultCompany ?? "Company name"}
                    value={company}
              ...
              <section>
                <StepLabel step={3} label="Pick an accent color" />
                <div className="mt-4 flex items-center gap-2 flex-wrap">
                  <Palette className="w-4 h-4 text-muted-foreground shrink-0" />
                  {COLOR_PRESETS.map((c) => {
                    const active = (selectedColor?.hsl ?? scenario?.defaultColor) === c.hsl;
                    return (
                      <button
                        key={c.hsl}
                        title={c.label}
                        ...
                        className={`w-7 h-7 rounded-full border-2 transition-transform focus:outline-none ${
                          active ? "border-white scale-125 ring-2 ring-white/30" : "border-transparent hover:scale-110"
                        }`}
                      />
                    );
                  })}
                  <span className="text-xs text-muted-foreground font-mono ml-2">
                    {(selectedColor?.label) ?? COLOR_PRESETS.find((c) => c.hsl === scenario?.defaultColor)?.label ?? ""}
                  </span>
                </div>
              </section>
              ...
              {/* Preview strip */}
              <div
                className="rounded-lg border border-border bg-card px-5 py-3 flex items-center justify-between"
                style={{ "--tw-ring-color": `hsl(${effectiveColor.hsl})` } as React.CSSProperties}
              >
                <div className="flex items-center gap-3">
                  <div
                    className="w-2.5 h-2.5 rounded-full animate-pulse"
                    style={{ background: `hsl(${effectiveColor.hsl})` }}
                  />
                  <span className="font-mono text-sm" style={{ color: `hsl(${effectiveColor.hsl})` }}>
                    {effectiveCompany || scenario?.defaultCompany}
                  </span>
                ...
                <span
                  ...
                  style={{
                    borderColor: `hsl(${effectiveColor.hsl} / 0.5)`,
                    color: `hsl(${effectiveColor.hsl})`,
                    background: `hsl(${effectiveColor.hsl} / 0.1)`,
                  }}
                >
                  PREVIEW
                </span>
